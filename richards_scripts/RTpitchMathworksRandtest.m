...
% Parameters
Amp_thresh=1000; % above that sound amplitude no change in shift amount (parameter changes only during pauses)
Amp_thresh_high=5000; % new idea: the true threshold is Amp_thresh_high, but Amp_thresh must be passed before

Min_Dur_per_level_s=1; % Pitch shift stays at least Min_Dur_per_level_s seconds at the specified level
T_s=6; % time in seconds
...
for ii=1:T
    ...
    ymag       = abs(yfft);
    ymag(1:2)=0; ymag(end:end)=0;  ymag(WindowLen/2-WindowLen/8:WindowLen/2+WindowLen/8)=0; % high-pass filter
    ...
    % new
    if 0%did_pass_low && ymags(ii)>Amp_thresh_high %
        cs=cs+1;
        Ts(cs+1)=(ii-1)*AnalysisLen;
        Ys{cs}=yall2(Ts(cs):Ts(cs+1)); % shifted pitch
        Ys0{cs}=yall(Ts(cs):Ts(cs+1));
        if rem(cs,2)==0
    ...
    if 0%(~do_var && new_synth) || do_var %(do_var && rem(ii,4)==0)
        new_synt=0;
...
% figure(8);clf; [y,x]=hist(ymags,100);stairs(x,y); hold on;   plot(x,log_mult*log(1+x/log_div),'r');
