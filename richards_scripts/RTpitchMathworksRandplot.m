...
for j=1:1+do_var
  ...
  for i=size(f0s,1):-1:1
      if i==lev0
        ...
        plot(Dts{i,j}(1:l),1200*log(Means{i,j}(1:l)./Means{lev0,j}(1:l)),cols{i},'linewidth',lw);
     %    1200*log(SynthesisLens/AnalysisLen);
         subplot(312); hold on;
         plot(Dts{i,j},Stds{i,j},cols{i},'linewidth',lw);    
         subplot(313); hold on;
%         plot(Dts{i,j}(1:l),(Means{i,j}(1:l)-Means{lev0,j}(1:l))/,cols{i},'linewidth',lw);         
        plot(Dts{i,j}(1:l),100*1200*log(Means{i,j}(1:l)./Means{lev0,j}(1:l))/cents_pitches(i),cols{i},'linewidth',lw);
  end
