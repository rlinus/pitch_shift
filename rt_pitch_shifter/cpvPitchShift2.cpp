...
void cpvPitchShift(float pitchShift, float *indata, float *outdata)
{
    ...
    if(ifftFrameSize>=fftFrameSize){
        ...
        for(k=0; k < fftFrameSize; k++) {
            gOutputAccum[k] += interpOutput[k+b]*window[k]/(osamp*3/(double)8);
        }
    }else{
        int b = (fftFrameSize-ifftFrameSize)/2;
        for(k=0; k < ifftFrameSize; k++) {
            gOutputAccum[k+b] += interpOutput[k]*window[k]/(osamp*3/(double)8);
        }
    }
    
//     for(k=0; k < fftFrameSize; k++) {
//         gFFTworksp_real[k] = gFFTworksp_real[k]*window[k]/(osamp*3/(double)8);
//     }
